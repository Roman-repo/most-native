...
- **REQ-9 — Расширенные реакции:**
  - `src/components/EmojiPickerModal.tsx` — новый компонент. Bottom-sheet модальное окно с категориями эмодзи (😊 🐱 🍕 ⚽ ✈️ 💡 🔣), полем поиска по эмодзи, секцией «Недавние» (хранится в `AsyncStorage`, max 16 штук). Grid 8 колонок, `FlatList` с `removeClippedSubviews` для производительности.
  - `src/components/ReactionPicker.tsx` — кнопка «+» теперь открывает `EmojiPickerModal` вместо inline `ScrollView`.
- **REQ-21 — «Читать далее»:**
  - `src/components/MessageBubble.tsx` — для текстовых сообщений: `numberOfLines={8}`, `onTextLayout` определяет, превышает ли текст 8 строк. Если да — показывается кнопка «Читать далее», при тапе разворачивает полный текст, повторный тап — «Свернуть».
- **REQ-40 + REQ-50 — Анимации реакций:**
  - `AnimatedReactionBadge` (внутри `MessageBubble.tsx`) — новый подкомпонент. При появлении (mount) нового badge: spring-анимация `scale` 0→1. При тапе на badge: pulse-анимация `scale` 1→1.35→1. Используется `Animated` из React Native, `useNativeDriver: true`.
- `package.json` + `app.json` — bump версии 4.18.0.
...
**Проблема:**
- Скролл чата шёл рывками, пузыри подрагивали при движении пальцем
- В веб-версии скролл идеально плавный — `RenderManager` рендерит все сообщения в DOM без виртуализации, браузерный скролл нативный
- В RN `FlashList`/`FlatList` unmount/mount элементы при скролле, вызывая layout-пересчёты
- `expo-blur` с `experimentalBlurMethod="dimezisBlurView"` на Android — софтверный blur, который пересчитывается на каждый кадр скролла, когда контент движется под хедером/pinBar/editBar/replyBar
- Ручное отслеживание клавиатуры давало артефакты: инпут подпрыгивал, emoji и клавиатура конфликтовали

**Что сделано:**
- `src/screens/ChatScreen.tsx`:
  - `FlashList` → **`ScrollView`**. Все сообщения рендерятся целиком (`messages.map()`), как в веб-версии. Скролл — нативный, без unmount/mount элементов при движении.
  - **`BlurView` полностью убран** из хедера, pinBar, editBar, replyBar. Заменён на `View` с `backgroundColor: theme.bg2` (`rgba(15,12,41,0.78)`). В вебе blur — GPU-ускоренный `backdrop-filter`, в RN на Android — софтверный performance hog.
  - `msgPositionsRef` — `Map<msgKey, yPosition>` для навигации к закреплённым сообщениям (через `onLayout` каждого сообщения).
  - Ручное отслеживание клавиатуры (~25 строк `Keyboard.addListener`) **полностью удалено**. Вместо этого `KeyboardAvoidingView` (behavior="padding") из `react-native-keyboard-controller`.
  - `bottomPad` теперь **только для emoji-панели**.
  - `kbHeightRef` (мёртвый ref) удалён.
- **Стикеры и живые стикеры** — логика приведена к веб-версии:
  - `onSticker` — стикер добавляется в поле ввода (не отправляется сразу). При отправке, если текст в поле === стикер — отправляется как `sticker` в Firebase.
  - `onAnimSticker` — устанавливается `pendingAnimSticker`. Показывается плашка «✨ Живой стикер» над полем ввода. При нажатии отправить — отправляется как `animSticker`. При редактировании/ответе/выборе фото — pending сбрасывается.
- **Живые стикеры стали динамичными** (`AnimStickerWebView`):
  - `src/components/AnimStickerWebView.tsx` — новый компонент. Рендерит SVG с SMIL-анимацией (`<animate>`, `<animateTransform>`) через `WebView`.
  - `react-native-webview@13.12.5` установлен в зависимости.
  - `EmojiPanel.tsx` — превью живых стикеров в панели теперь анимированные.
  - `MessageBubble.tsx` — живые стикеры в чате теперь анимированные (вместо статичного `SvgXml`).
...
- Навигация к закреплённым сообщениям работает
